...
10件の学習データで評価した結果（IoU閾値: 0.5）：

| 指標 | 値 | 説明 |
|------|-----|------|
| **Recall (検出率)** | 6.21% | IoU≥0.5でマッチした正解の割合 |
| **Precision (適合率)** | 4.23% | IoU≥0.5でマッチした予測の割合 |
| **平均IoU** | 0.094 | 予測と正解のbbox重なり度合い |
| **マッチ数** | 11/177 | 正解177件中11件を正しく検出 |
...
⚠️ **現状の課題**: 学習データが10件と少なく、サンプルによって精度にばらつきがあります。サンプル7,9では比較的良い結果（Recall 40%超）が出ていますが、多くのサンプルでは座標のずれが大きい状態です。
