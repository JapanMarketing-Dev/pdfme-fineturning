...
| IoU閾値 | Recall | Precision | マッチ数 | 説明 |
|---------|--------|-----------|----------|------|
| **0.5（厳密）** | 6.21% | 4.23% | 11/177 | 正確な位置で検出 |
| **0.3（緩め）** | 11.86% | 8.08% | 21/177 | おおよその位置で検出 |

IoU 0.3では精度が約2倍に向上 → **モデルは「おおよその位置」は捉えているが、正確な境界ではない**

#### サンプル別の結果（IoU≥0.5）

| サンプル | 正解数 | 予測数 | Recall | Precision | 平均IoU |
|---------|--------|--------|--------|-----------|---------|
| 0 | 30 | 31 | 0.00% | 0.00% | 0.000 |
| 1 | 20 | 6 | 0.00% | 0.00% | 0.044 |
| 2 | 15 | 37 | 6.67% | 2.70% | 0.147 |
| 3 | 16 | 37 | 0.00% | 0.00% | 0.024 |
| 4 | 18 | 38 | 0.00% | 0.00% | 0.000 |
| 5 | 12 | 30 | 8.33% | 3.33% | 0.086 |
| 6 | 20 | 18 | 5.00% | 5.56% | 0.126 |
| **7** | **12** | **27** | **41.67%** | **18.52%** | **0.363** |
| 8 | 23 | 6 | 0.00% | 0.00% | 0.057 |
| **9** | **11** | **30** | **27.27%** | **10.00%** | **0.355** |

#### 結果の分析

- **良好なサンプル（7,9）**: Recall 27-42%、IoU 0.35以上 → 座標のずれが小さい
- **失敗したサンプル**: 座標のX軸が大きくずれている傾向
- **全体的な傾向**: Y座標は比較的正確、X座標がずれやすい

⚠️ **現状の課題**: 学習データが10件と少なく、サンプルによって精度にばらつきがあります。データ拡張（10件→90件）による再学習で改善が期待できます。

### 評価指標の定義

- **Recall（検出率）**: 正解フィールドのうち、IoU≥閾値で検出できた割合
- **Precision（適合率）**: 検出したフィールドのうち、IoU≥閾値で正解だった割合
- **IoU（Intersection over Union）**: 予測bboxと正解bboxの重なり具合（0〜1）

### 評価スクリプト

```bash
# 評価を実行（IoU閾値0.5）
python src/evaluate.py --lora outputs/pdfme_lora_YYYYMMDD_HHMMSS --verbose

# IoU閾値を変更（0.3で緩い評価）
python src/evaluate.py --lora outputs/pdfme_lora_YYYYMMDD_HHMMSS --iou-threshold 0.3
```
